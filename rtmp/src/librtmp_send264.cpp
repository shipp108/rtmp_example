...
/*
	...
	unsigned char *msg = (unsigned char *)sps;
	for (int kk = 0; kk < sps_len; kk++) {
		if ((kk + 1) % 16 == 0) {
			printf("\n");
		}
		printf("0x%02x ", msg[kk]);
	}
	printf("\n=======================\n");
	msg = (unsigned char *)pps;
	for (int kk = 0; kk < pps_len; kk++) {
		if ((kk + 1) % 16 == 0) {
			printf("\n");
		}
		printf("0x%02x ", msg[kk]);
	...
	printf("\n=======================\n");
	msg = (unsigned char *)packet->m_body;
	for (int kk = 0; kk < packet->m_nBodySize; kk++) {
		if ((kk + 1) % 16 == 0) {		
			printf("\n");
		}
		printf("0x%02x ", msg[kk]);
...
{    
	...
	for (int i = 0; i < metaData.nSpsLen; i++) {
		printf("0x%02x ", metaData.Sps[i]);
	}
	printf("=============-+===========\n");	
